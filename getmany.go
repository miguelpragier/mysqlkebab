...
func (l *DBLink) GetMany(sqlQuery string, args ...interface{}) (Resultset, error) {
	...
	if err != nil {
		if l.debugPrint {
			log.Printf(`mysqlkebab.GetMany.db.Query("%s") has failed: "%v"\n`, sqlQuery, err)
		}
	...
	if err0 != nil {
		if l.debugPrint {
			log.Printf(`mysqlkebab.GetMany.rows.Columns("%s") has failed: "%v"\n`, sqlQuery, err0)
		}
	...
	if colTypes, err8 := rows.ColumnTypes(); err8 != nil {
		if l.debugPrint {
			log.Printf(`mysqlkebab.GetMany.rows.ColumnTypes("%s") has failed: "%v"\n`, sqlQuery, err8)
		}
	} else {
	...
	for rows.Next() {
		...
		if er2 := rows.Scan(fakeDest...); er2 != nil {
			if l.debugPrint {
				log.Printf(`mysqlkebab.GetMany.rows.Scan("%s") has failed: "%v"\n`, sqlQuery, er2)
			}
